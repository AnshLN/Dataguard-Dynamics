<template>
    <formPageDesign :page-data="CMSMenuNewData">
        <div class="row">
            <div class="col-lg-5">
        ...
        <!-- Button trigger modal -->
            ...
            <!-- Modal -->
            <div class="modal fade" id="dataSaveSuccess" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content border-0">
                    <div class="alert alert-success mb-0" role="alert">
                        Menu Added Successfully Visit <a href="#" @click.prevent="goToTable()" class="alert-link">CMS Menu</a> to see the updates.
                    </div>
          ...
          return{
            CMSMenuNewData:{
                breadcrumbs: [
                    ...
                    { name: 'New', path: '/admin/cms/appearance/menu/new'}                   //follow the sequence of breadcrumbs , last item of breadcrumbs will remain ACTIVE
                ],
                caption_icon: 'fa-solid fa-plus',
                caption_title: 'ADD MENU',
                action_links:[ 
                ]
            },
            FormData:{
    ...
    Edit(id){
      ...
      async saveToJson(){
        ...
        if(NameValidity && SlugValidity && MenuTypeValidity && MenuItemsValidity){
            ...
            try {
            const response = await axios.post('/api/CMSMenuTableData', {
                "menu_name": this.Name,
                "status": this.Enable_Disabled ? "Enabled" : "Disabled",
                "id": "unique_key_" + Date.now(),
                "Slug": this.Slug,
